    ...
    for i in range(len(parent1)):
        if child1[i] == -2:
            ...
            while corresponding_gene in child1[start:end + 1]:
                idx = np.where(parent1 == corresponding_gene)[0]
                if len(idx) > 0:
                    corresponding_gene = parent2[idx[0]]
                else:
                    break
            child1[i] = int(corresponding_gene)

        if child2[i] == -2:
            corresponding_gene = parent1[i]
            while corresponding_gene in child2[start:end + 1]:
                idx = np.where(parent2 == corresponding_gene)[0]
                if len(idx) > 0:
                    corresponding_gene = parent1[idx[0]]
                else:
                    break
            child2[i] = int(corresponding_gene)
