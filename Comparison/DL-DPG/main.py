...
from DLDPG import DLDPG_train, DLDPG_test
                        ...
                        for d_n in drone_Number:
                            for u_n in user_Number:
                                for k in range(1):
                                    ...
                                    satisfaction0, Groups, drones,satisfaction = DLDPG_test(users, drones, Groups)
                                    ...
                                    content = [content_name] + ['DL-DPG'] + Argument + [
                                        f'{satisfaction:.1f}'] + [f"{(time.time() - start):.0f}"]+[f"{satisfaction0:.1f}"]
                                    # # 使用'a'模式打开文件以进行追加写入
                                    # 直接全部保存到一个文件
