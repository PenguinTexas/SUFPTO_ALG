...
import pickle
import time
from SA_Offloading_and_Scheduling import \
    Joint_Task_Offloading_and_UAV_Scheduling_Optimization_Algorithm, Joint_scheduling_offloading
                        ...
                        for d_n in drone_Number:
                            for u_n in user_Number:
                                for k in range(5):
                                    ...
                                    satisfaction, Groups, drones = Joint_scheduling_offloading(users, drones, Groups,
                                                                                               'p', 'SA', 'SA')
                                    ...
                                    content = [content_name] + ['SA'] + Argument + [f'{satisfaction:.1f}'] + [
...
if __name__ == '__main__':
    # 729=3*3*3*3*3*3
    drone_B = [2]  # MHz,=*1e6Hz
    drone_energy = [70]  # Wh,=*3600J
    user_computing_power = [(4, 6)]  # MHz,=*1e6Hz
    drone_computing_power = [150]  # MHz,=*1e6Hz
    drone_velocity = [50]  # m/s
    drone_number = [4]
    user_number = [100]
