...
def E_i_k(Group):
    ...
    # print("飞行能耗：",E_flight_total)
    # print("下行结果能耗：", E_down_total)
    # print("计算能耗：", E_comp_total)
    # print("悬停能耗：", E_hover_total)
...
# 替换原有的 energy_constraint 函数
def energy_constraint(drone):
    """无人机能量约束检查（统一版本）"""
    if len(drone.serve_group) == 0:
        return True
    if drone.serve_group[-1].drone_k is None:
        return False
    E_total = sum(E_i_k(g) for g in drone.serve_group)
    E_total += E_back(drone.serve_group[-1])
    return E_total < drone.Emaxk

def energy_total_constraint(drones):
    """所有无人机能量约束检查"""
    return all(energy_constraint(d) for d in drones)
