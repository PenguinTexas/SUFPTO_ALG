...
plt.rcParams['font.sans-serif'] = ['SimHei']  # 指定使用微软雅黑字体
...
def simulated_annealing(G, K, x, Groups):
    ...
    while t < iteration:
        ...
        # fitness_history.append(-current_cost)
        # 降低温度
        temperature *= cooling_rate
        # print(f"此时温度为：{temperature}")
...
def initialize_population(population_size, G):
    ...
    for i in range(population_size):
        ...
        # 使用SA生成初始种群
        # a = time.time()
        # solution = simulated_annealing_initialize(solution, K, x, Groups)
        # print(f"生成一条染色体的时间：{time.time() - a}")
...
def GA_for_Balanced_Assignment_Problem(G, K, x, Groups):
    ...
    # print(f"初始化种群需要的时间为：{time.time() - i1}")
    # population = initialize_population(population_size, G)
...
def GA_based_UAV_Scheduling_Algorithm(drones, Groups, assign, scheduling):
    ...
    while len(C) != 0 and len(K) != 0:
        ...
        for k in K_delete:
            K = K[K != k]
        # print(f"    本次调度分配出去的组位：{[s.h for s in S]}")
        # print(f"    所有剩余可分配的组C:{[c.h for c in C]}")
        # print(f"    剩下有能量的无人机K(t):{[a.k for a in K]}")
