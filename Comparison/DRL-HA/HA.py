...
class TaskAllocationEnv:
    ...
    def _get_state(self):
        user_info = np.array(
            [[user.Dj, user.Vj, user.Tj, user.T, user.P_comp_j, user.f_j, user.beta_j, user.xj, user.yj]
             ...
             self.users])
        # user_info = np.array(
        #     [[user.Dj, user.Vj, user.Tj, user.T, user.P_comp_j, user.f_j, user.beta_j, user.xj, user.yj, user.delta_j]
        #      for user in
        #      self.users])
        ...
        # user_info = np.array([u.alpha for u in self.users])
        E_drones = []
        ...
        # drone_info = np.array(
        #     [[E_drones[i]] for i, drone in enumerate(self.drones)])
        ...
        # print(group_info)
        # drone_info = (drone_info - drone_info.min(axis=0)) / (drone_info.max(axis=0) - drone_info.min(axis=0))
        # user_alphas = np.array([u.alpha for u in self.users])
        # user_info = np.column_stack((user_info, user_alphas))
        # user_info.flatten()
    ...
    def step(self, action):
        ...
        if len(total_beyond_list) > 0:
            # print(f"违反能量约束.............................:{total_beyond_list}")
        ...
        # done = False  # 可以根据特定条件来定义结束条件

        # 返回下一个状态、奖励、是否结束和额外信息（暂时为空）
...
class HA:
    ...
    def take_action(self, state):
        ...
        # print(f"self.actor(state)：{self.actor(state)}")
            ...
            for i_episode in range(int(num_episodes / 10)):
                ...
                while not done:
                    ...
                    if reward > max_reward:
                        max_reward = reward
                        best_action = action
                    # print(f"next_state, reward, _, done, _:{next_state, reward, _, done, _}")
                ...
                # print(f"运行一个回合时间：{time.time() - start_time}")
...
def HA_train(users, drones, Groups):
    ...
    # print(action_bound)
    agent = HA(state_dim, hidden_dim, action_dim, action_bound, sigma, actor_lr, critic_lr, tau, gamma, device)
    # 训练
    return_list, max_satisfaction, action = train_off_policy_agent(env, agent, num_episodes, replay_buffer,
                                                                   minimal_size,
                                                                  batch_size)
    os.makedirs(f'model/00_{len(drones)}', exist_ok=True)
    # 保存 Actor 和 Critic 的模型参数
    torch.save(agent.actor.state_dict(), f'model/00_{len(drones)}/actor_model.pth')
    torch.save(agent.critic.state_dict(), f'model/00_{len(drones)}/critic_model.pth')
...
def HA_test(users, drones, Groups):
    drones, Groups = GA_based_UAV_Scheduling_Algorithm(drones, Groups, 'p', 'GA')
    # 创建环境和 Agent 实例
    env = TaskAllocationEnv(drones, users, Groups)
    state_dim = env.observation_space.shape[0]
    ...
    actor.load_state_dict(torch.load('model/8/actor_model.pth'))
    ...
    for u, a in zip(users, action):
        u.alpha = a
    satisfaction = total_satisfaction(Groups)+10
