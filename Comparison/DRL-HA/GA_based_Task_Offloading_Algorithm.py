...
def fitness_function(solution, users, drones):
    total_cost = 0
    ...
    if not energy_total_constraint(drones):
        return False
    for drone in drones:
        for group in drone.serve_group:
...
def hill_climbing(Ns2, initial_solution, users, drones):
    # start_time = time.time()
    current_solution = initial_solution.copy()
    for i in range(Ns2):
        # 计算当前解的适应度（目标函数值）
        current_fitness = fitness_function(current_solution, users, drones)
        n = 0
        attempts = 0
        ...
        # print("自优化循环次数：", n)
        # 计算邻近解的适应度
        neighbor_fitness = fitness_function(neighbor_solution, users, drones)
        # 如果邻近解更好，则接受它作为新的当前解
        if neighbor_fitness > current_fitness:
            current_solution = neighbor_solution
        end_time_a = time.time()
        # print(f"自优化爬一次需要时间：{end_time_a - start_time_a}")
    end_time = time.time()
    # print(f"自优化的运行时间为:{end_time - start_time}")
...
def mutate(child, Nm2, users, drones):
    ...
    while True:
        ...
        if solution_constraint(child_0, users, drones):  # 判断满足无人机最大能量约束
            end_time = time.time()
            # print(f"突变操作的运行时间为:{end_time - start_time}")
            return child_0
        else:
            attempts += 1
        if attempts == max_attempts:
...
def GA_based_Task_Offloading_Algorithm(users, drones, population_size, Niter2, crossover_rate, mutation_rate, Ns2, Nm2):
    last_alpha = [u.alpha for u in users]  # 记录上次迭代结果
    print(f"    任务卸载优化.............")
    population = initialize_population(population_size, users, drones)
    # population = simulated_annealing_initialize(population_size, users, drones)
    # 主循环：遗传算法的迭代过程
    for r in range(Niter2):  # 迭代索引
        ...
        for individual in population:
            fitness = fitness_function(individual, users, drones)
        ...
        for i in range(elite_size):
            elite[i] = hill_climbing(Ns2, elite[i], users, drones)
        # 将优化后的精英个体添加到新种群
        # 生成新种群
    ...
    best_solution = max(population, key=lambda y: fitness_function(y, users, drones))
    this_best = max([best_solution, last_alpha], key=lambda y: fitness_function(y, users, drones))
